 ...
 *
 */
class Absenm4Form extends Model
{
    public $inputan;
    ...
    public function addForm($params)
    {
        ...
        $daftarVerifikasi1 = Verifikasi::findAll(['pendaftaran_noPendaftaran' => $this->noPendaftaran, 'tahapVerifikasi_id' => 4]);
        $daftarVerifikasi2 = Verifikasi::findAll(['pendaftaran_noPendaftaran' => $this->noPendaftaran, 'tahapVerifikasi_id' => 5]);
        ...
        elseif (empty($daftarVerifikasi1)) {
            $msg = 'Silahkan Melakukan Pemeriksaan Bukti Pembayaran di M3 Terlebih dahulu';
            $typemsg = 'error';
        } elseif ($daftarVerifikasi2 != null) {
            $msg = 'Sudah Terdaftar di M4';
            $typemsg = 'warning';
        } else {
            $daftarVerifikasi = Verifikasi::findAll(['pendaftaran_noPendaftaran' => $this->noPendaftaran, 'tahapVerifikasi_id' => 5]);
...
//            }
            ...
            if (empty($daftarVerifikasi)) {
                ...
                $daftarVerifikasi->tahapVerifikasi_id = empty(Tableverifikasi::findOne(['name' => 'M4'])->id) ? 5 :
                    Tableverifikasi::findOne(['name' => 'M4'])->id;
                ...
                if ($daftarVerifikasi->save()) {
                    $msg = 'Mahasiswa berhasil masuk M4';
                    $typemsg = 'success';
                } else {
                ...
                if ($daftarVerifikasi->paketVerifikasi_id->aktiv != 1) {
                    ...
                    if ($daftarVerifikasi->save()) {
                        $msg = 'Mahasiswa berhasil masuk M4, Mahasiswa sudah melakukan absen tahap $thp';
                        $typemsg = 'warning';
                    } else {
                        $msg = 'tidak dapat menginput data hubungi staf IT';
                        $typemsg = 'error';

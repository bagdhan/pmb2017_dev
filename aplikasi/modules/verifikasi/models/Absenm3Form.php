 ...
 *
 */
class Absenm3Form extends Model
{
    public $inputan;
    ...
    public function addForm($params)
    {
        ...
        date_default_timezone_set("Asia/Jakarta");
         $daftarVerifikasi1 = Verifikasi::findAll(['pendaftaran_noPendaftaran' => $this->noPendaftaran, 'tahapVerifikasi_id' => 3]);
        $daftarVerifikasi2 = Verifikasi::findAll(['pendaftaran_noPendaftaran' => $this->noPendaftaran, 'tahapVerifikasi_id' => 4]);
        ...
        elseif (empty($daftarVerifikasi1)) {
            $msg = 'Silahkan Melakukan Pemeriksaan Dokumen di M2 terlebih dahulu';
            $typemsg = 'error';
        } elseif ($daftarVerifikasi2 != null) {
            $msg = 'Anda Sudah Terdaftar di M3';
            $typemsg = 'warning';
        }

        else {
            $daftarVerifikasi = Verifikasi::findAll(['pendaftaran_noPendaftaran' => $this->noPendaftaran, 'tahapVerifikasi_id' => 4]);
...
//            }
            ...
            if (empty($daftarVerifikasi)) {
                ...
                $daftarVerifikasi->tahapVerifikasi_id = empty(Tableverifikasi::findOne(['name' => 'M3'])->id) ? 4 :
                    Tableverifikasi::findOne(['name' => 'M3'])->id;
                ...
                if ($daftarVerifikasi->save()) {
                    $msg = 'Mahasiswa berhasil masuk M3';
                    $typemsg = 'success';
                } else {
                    $msg = 'tidak dapat menginput data hubungi staf IT';
                    $typemsg = 'error';
                }
            } elseif ($daftarVerifikasi = 1) {
                ...
                if ($daftarVerifikasi->paketVerifikasi->aktiv != 1) {
                    ...
                    if ($daftarVerifikasi->save()) {
                        $msg = 'Mahasiswa berhasil masuk M3, Mahasiswa sudah melakukan absen tahap $thp';
                        $typemsg = 'warning';
                    } else {
                        $msg = 'tidak dapat menginput data hubungi staf IT';
                        $typemsg = 'error';
